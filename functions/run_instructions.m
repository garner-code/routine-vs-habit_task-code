function [] = run_instructions(window, screenYpixels, stage, house)

Screen('TextStyle', window, 1);
Screen('TextSize', window, 20);
instruct_col = [49, 130, 189];

if stage == 1 
    if house == 1
    instructions = ...
        sprintf(['Animals are playing hide & seek in a house with 16 rooms.\n\n'...
                 'Your task is to find the animals hiding in these rooms. \n\n'...
                 'You will click on a door to look inside a room.\n\n'...
                 'Press any key to see the doors and have a practice.\n']);
    elseif house == 2
      instructions = ...
        sprintf(['Time to learn the next house!\n'...
                'Remember to try and find the animals in 4 moves or less\n']);
    elseif house == 9
      instructions = ...
        sprintf(['We`re going to go through that one more time\n'...
                'Remember to try and find the animals in 4 moves or less\n']);
    end
elseif stage == 2 
    instructions = ...
        sprintf(['Nice work!\n\n' ...
        'Now, it will be more difficult to tell which house you are in.\n'...
        'Because the coloured borders will go away.\n\n'...
        'You must remember the animals hiding places, so that \n'...
        'you can tell which house you are in\n\n' ...
        'The house will change from time to time. However the house \n' ...
        'will always stay the same far more often that it changes.\n\n' ...
        'You will now have the opportunity to practice finding the animals. \n'...
        'Keep trying to find the animals in 4 moves or less.\n\n' ...
        'Good luck! You got this...\n' ...
        'Press any key to start the task.'

]);
% elseif stage == 3 && experiment == 1
%     instructions = ...
%         sprintf(['Way to go!\n\n' ...
%         'Now, you will be able to use the colours to tell \n' ...
%         'you which house you are in.\n\n' ...
%         'Press any key to start. Good luck!']);
% elseif stage == 3 && experiment == 2
%     instructions = ...
%         sprintf(['Way to go!\n\n' ...
%         'Now, the animals have moved to two new houses.\n' ...
%         'Your task is still to find the animals.\n\n' ...
%         'It may help to use their previous hiding \n' ...
%         'places to find them.\n\n' ...
%         'Press any key to start. Good luck!']);
end

DrawFormattedText(window, instructions,'Center', screenYpixels*.3, instruct_col);
Screen('Flip', window);

end

        % 'The good news is that now each time you hear the tones, you will win points!\n'...,
        % 'Points will convert to cold hard cash bonuses!\n'...
        % 'You will reap rewards when you find the animal in 4 moves or less.\n'...
        % '1 move = 4 points\n'...
        % '2 moves = 3 points\n'...
        % '3 moves = 2 points\n'...
        % '4 moves = 1 points\n\n'...
        % 'You will know how many points you won from how many tones you hear.\n\n'...
        % 'Note that points will not be available on every trial. If you get the animal\n'...
        % 'In less than 4 moves and you don`t hear a sound, then there were no points that\n'...
        % 'time\n'...
        % 'Remember that points become sweet cash!'